...
try:
    ...
    def dump_joblib(p: Path, obj): p.parent.mkdir(parents=True, exist_ok=True); joblib.dump(obj, p)
...
def main():
    ap = argparse.ArgumentParser(description="Algolab ML Runner（自动/指定任务，带清洗日志）")
    ...
    if clean_cfg:
        ...
        if probe_col in df.columns:
            ...
            # 找不到就跳过，避免 KeyError
            print(f"\n[Task Probe | before clean] 找不到列 '{args.target}'（或其重命名前的列），跳过。"
                  f" 现有列示例：{list(df.columns)[:20]}")

        # 进行清洗（这里会执行 rename 到 label）
        df = _apply_cleaning_with_log(df, clean_cfg, target=args.target)

        # —— 清洗后再探测一次，应该能直接用最终目标名
